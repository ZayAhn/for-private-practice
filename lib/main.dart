...
final todosProvider = ChangeNotifierProvider<DataClass>((ref) {
  return DataClass();
});
...
class Texts{
  ...
  Texts({required this.text});
}

class DataClass extends ChangeNotifier{
 final texts= <Texts>[];
 // 공문 인용해서 쓴 함수 이 함수가 Ui에게 데이터를 추가 시켜주는 것같은데 왜??
 // Let's allow the UI to add todos.
 void addTodo(Texts description) {
...
class TextForm extends HookConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    List<Texts> textreader = ref.watch(todosProvider);
